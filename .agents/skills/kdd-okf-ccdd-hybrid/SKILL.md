...
Además, dos herramientas de diagnóstico opt-in (**no son gates, no corren en CI; Nivel 1 sigue siendo 11 gates**) documentadas en [knowledge/validacion.md](../../../knowledge/validacion.md) (secciones "Preflight" y "Auditor de seals débiles"): `python scripts/preflight.py` (dry-run local de los 12 gates; úsalo antes de delegar o empezar trabajo) y `python scripts/audit_seals.py` (auditor ADVISORY de "seals débiles"; úsalo al autorar o revisar un oráculo antes de sellarlo — el sello garantiza integridad, no fuerza).
